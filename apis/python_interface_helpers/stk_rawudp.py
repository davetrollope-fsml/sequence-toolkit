...
class stk_rawudp_listener:
	def __init__(self,env,name,id,options,ref=None):
		if ref == None:
			self._df = stk_rawudp_listener_create_data_flow(env.ref(),name,id,options.ref())
			if self._df == None:
...
class stk_rawudp_client:
	def __init__(self,env,name,id,options,ref=None):
		if ref == None:
			self._df = stk_rawudp_client_create_data_flow(env.ref(),name,id,options.ref())
			if self._df == None:
				raise Exception("Failed to create rawudp client data flow")
		else:
			self._df = ref
		self._id = stk_get_data_flow_id(self._df)
		self._dfptr = stk_df_ptr_to_ulong_df(self._df)
...
class stk_rawudp_publisher(stk_rawudp_client):
	pass
